...
from midi_synthesizer import notes2midi
...
if __name__ == "__main__":
    filename_musicxml = "/Users/ronggong/Documents_using/pycharmProjects/piano-interative-demo/data/Minuet_in_G_Major_Bach_first_16bars_musicxml"
    # load score of sonic visualizer
    score_s = sv_score_parser(filename_musicxml+".txt")
    score_t = mid_note_parser("/Users/ronggong/Documents_using/pycharmProjects/piano-interative-demo/data/BACH_Minuet_in_G_Major_first16bars.wav.midi")
    
    notes2midi(score_s, filename_musicxml+".midi")
    save_midi_2_audio(filename_musicxml+".midi", filename_musicxml+".wav")
    
    y_t, sr_t = librosa.load("/Users/ronggong/Desktop/BACH_Minuet_in_G_Major_first16bars.wav", sr=None)
    y_s, sr_s = librosa.load(filename_musicxml+".wav", sr=None)

    list_score_aligned = alignment(y_t, y_s, sr_t, sr_s, score_t, score_s, plot=True)

    with open("./data/alignment.txt", "w") as f:
        for note_t_s in list_score_aligned:
            if not note_t_s[1]:
                note_t_s[1] = [None, None, None, None]
            if not note_t_s[0]:
                note_t_s[0] = [None, None, None, None]
            f.write(str(note_t_s[1][0])+'\t'+str(note_t_s[1][1])+'\t'+str(note_t_s[1][2])+'\t'+str(note_t_s[1][3])+
                    '\t'+str(note_t_s[0][0])+'\t'+str(note_t_s[0][1])+'\t'+str(note_t_s[0][2])+'\t'+str(note_t_s[0][3])+'\n')